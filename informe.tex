...
\begin{ejercicio}
	...
	\begin{problema}{Dividir}{im : Imagen, m, n : \ent}{listaPartes:[Imagen]}
		...
		\asegura {todos([alto(parte)$ $==$ $alto(im)$ $div$ $m$ $\land$ $ancho(parte)==ancho(im)$ $div$ $n$ $|$ $parte \leftarrow listaPartes])}
	\end{problema}
...
\begin{ejercicio}
	...
	\begin{problema*}{Pegar}{destino : Imagen, col : Pixel, origen: Imagen}
		...
		\asegura { \lnot imagenTieneHuecoPegar(pre(destino), origen, col) \Then destino == pre(destino)}
    	\asegura {imagenTieneHuecoPegar(pre(destino), origen, col) \Then aplicaPegar(pre(destino), destino, huecoPegar(origen, col), origen)}
	\end{problema*}
...
\item \auxil{aplicaPegar(grande1, grande2, chica1, chica2 : Imagen) : Bool}{
	\\ mismoTama\tilde{n}o(grande1, grande2) \ \land \ mismoTama\tilde{n}o(chica1, chica2)\ \land \
		\\ alguno([
		chica1 == subImagen(x0,  x0  +  ancho(chica1)  - 1, y0 , y0  + alto(chica1) -  1,  grande1) \ \land \ chica2 == subImagen(x0, x0 + ancho(chica2) - 1, y0, y0 + alto(chica2) - 1, grande2) \ \land \ \\ sonIgualesSalvoRectangulo(grande1, grande2, x0, x0+ancho(chica1) - 1, y0, y1+alto(chica1) - 1)\ |$ $x0 \leftarrow [0...ancho(grande1) - ancho(chica1)], y0 \leftarrow [0...alto(grande1) - alto(chica1)]	
])
}
